...
//Cabe destacar que tanto el gameprinter como el MyStringUtils los hemos obtenido de la asignatura TP I
public class GamePrinter {
	...
	public GamePrinter (Tablero t, int rows, int cols) {
		this.numRows = rows;
		this.numCols = cols;		
		encodeGame(t);
	}
	
	private void encodeGame(Tablero t) {
		board = new String[numRows][numCols];
		for(int i = 0; i < numRows; i++) {
			for(int j = 0; j < numCols; j++) {
				board[i][j] = t.positionToString(i,j);
			}
		}
	}
